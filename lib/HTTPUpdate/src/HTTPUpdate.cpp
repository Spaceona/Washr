...
/**
 * I updated this file to accept a server connection in a format that I like better.
 * -Robbie Leslie
 */
...
//This is the version of the function that I updated to just accept a complete server string -Robbie
HTTPUpdateResult HTTPUpdate::update(WiFiClient& client, String firmwareServer, HTTPUpdateRequestCB requestCB)
{
    HTTPClient http;
    if(!http.begin(client, firmwareServer))
    {
        return HTTP_UPDATE_FAILED;
    }
    return handleUpdate(http, "", false, requestCB);
}
